...
void Visualizer::AddNewPoseToPath(
  ...
  if(time_double < 0){
    ...
    int64_t nsec = static_cast<int64_t>(pose_stamped.header.stamp.nanosec);
...
void Visualizer::PublishImage(cv::Mat& image, double time_double){
  sensor_msgs::msg::Image::SharedPtr image_msg = cv_bridge::CvImage(std_msgs::msg::Header(), "mono8", image).toImageMsg();
  if(time_double > 0){
